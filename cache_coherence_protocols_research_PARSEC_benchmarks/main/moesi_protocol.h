#ifndef MOESI_PROTOCOL_H
#define MOESI_PROTOCOL_H
...
namespace MOESI {
    ...
    enum class MOESIState { INVALID, SHARED, EXCLUSIVE, MODIFIED, OWNED };
    ...
    // Standard LRU Cache Set (Same as MESI)
    class LRUCacheSet {
        int assoc;
    ...
    public:
        ...
        std::pair<CacheLine*, std::optional<CacheLine>> allocate(uint32_t tag) {
            ...
            lru_order.push_front(tag);
            lines[tag] = {tag, MOESIState::INVALID};
    ...
    public:
        MOESICache(int size, int assoc, int bsize) : block_size(bsize) {
    ...
    private:
        std::vector<MOESICache> caches;
        int num_cores, block_size;
        GlobalStatistics& g_stats;
        std::vector<Statistics>& c_stats;

    public:
        MOESIProtocol(int n, int cs, int as, int bs, GlobalStatistics& gs, std::vector<Statistics>& cs_ref) 
        ...
        ProtocolResponse handle_processor_request(int id, uint32_t addr, bool is_write) override {
            ...
            if (line && line->state != MOESIState::INVALID) {
                if (is_write) {
                    // Write to Shared or Owned requires Upgrade
                    if (line->state == MOESIState::SHARED || line->state == MOESIState::OWNED) {
                ...
                // Stats
                if (line->state == MOESIState::SHARED || line->state == MOESIState::OWNED) c_stats[id].shared_accesses++;
                else c_stats[id].private_accesses++;
        ...
        int handle_bus_transaction(int req_id, BusTransactionType type, uint32_t addr) override {
            ...
            for (int i = 0; i < num_cores; i++) {
                ...
                if (snoop_line && snoop_line->state != MOESIState::INVALID) {
                    shared_signal = true;
                    
                    if (snoop_line->state == MOESIState::MODIFIED || 
                        snoop_line->state == MOESIState::EXCLUSIVE || 
                        snoop_line->state == MOESIState::OWNED) {
                        supplier_found = true;
                    }

                    if (type == BusTransactionType::BUS_RDX || type == BusTransactionType::BUS_UPGR) {
                        snoop_line->state = MOESIState::INVALID;
                        ...
                        // MOESI Optimization:
                        // If M, transition to O (Owned) and supply data. NO WRITEBACK.
                        if (snoop_line->state == MOESIState::MODIFIED) {
                            snoop_line->state = MOESIState::OWNED;
                        } 
                        // If E, transition to S.
                        else if (snoop_line->state == MOESIState::EXCLUSIVE) {
                            snoop_line->state = MOESIState::SHARED;
                        }
                        // If O, stay O and supply data.
                    }
                }
            }
            ...
            if (type == BusTransactionType::WRITEBACK) {
                ...
                caches[req_id].get_line(addr)->state = MOESIState::MODIFIED;
            } else {
                // BusRead/RdX
                ...
                // Eviction of M or O requires Writeback
                if (evicted && (evicted->state == MOESIState::MODIFIED || evicted->state == MOESIState::OWNED)) {
                    latency += 100;
                    g_stats.bus_writebacks++;
                    g_stats.total_data_traffic_bytes += block_size;
                }
                ...
                if (type == BusTransactionType::BUS_RD) {
                    line->state = shared_signal ? MOESIState::SHARED : MOESIState::EXCLUSIVE;
                } else {
                    line->state = MOESIState::MODIFIED;
                }
            }
        ...
        void print_report(uint64_t total_cycles) override {
            std::cout << "===== MOESI Protocol Simulation Results =====" << std::endl;
